...
library(ranger)
set.seed(1500)
rf_model = ranger(SalePrice ~ ., data = as.data.frame(training(home_split), na.rm=TRUE))
rf_pred = predict(g1, testing(home_split))$predictions
```

```{r}

X = makeX(train = training(home_split) %>% select(-SalePrice),
          test = testing(data_split) %>% select(-SalePrice),
)
set.seed(1500)
glm_model = cv.glmnet(X$x, training(home_split)$SalePrice)
glm_pred = predict(glm_mode, X$xtest, s = "lambda.min") 
```
