...
::: {.callout-note title="Solution"}
1.  Since the data is an exponential distribution, $f(x,θ) = θe^{-θx}$ for all $x \geq 0$

2.  $L(θ) = \prod_{i=1}^nθe^{-θx_i}=θ^n\prod_{i=1}^ne^{-θx_i}$

3.  $ln(L(θ)) = n(ln(θ))-θ\sum_{i=1}^nx_i$

4.  $\frac{dln(L(θ))}{dθ}=\frac{n}{θ}-\sum_{i=1}^nx_i = 0$

5.  $\frac{n}{θ}=\sum_{i=1}^nx_i$

6.  $θ_{ML} = \frac{n}{\sum_{i=1}^nx_i}$
:::
...
::: {.callout-note title="Solution"}
```{r}
ggplot(model_data, aes(x=X,y=Y))+geom_point(color=Z)
```
:::
...
::: {.callout-note title="Solution"}
```{r}
prop.test(95,102, conf.level = .9)
```

The 90% confidence interval for the probability a patient is correctly classified is between 87.2% and 96.6%.
:::
...
::: {.callout-note title="Solution"}
```{r}
estate_data = read.csv("https://raw.githubusercontent.com/uvastatlab/phdplus/master/data/albemarle_homes.csv")
est_model <- lm(totalvalue~condition+finsqft+city, data = estate_data)
summary(est_model)
new_data <- data.frame(finsqft = c(2500, 1850),
       city = c('EARLYSVILLE', 'CROZET'),
       condition = c('Good', 'Fair'))
predict(est_model, newdata = new_data)
```
:::
...
Based on this t-test, we can conclude that *Action* movies are ranked lower on average than *Romance* movies.
:::
